...
2. **Более коварный баг: сканер молча пропускал уязвимости в файлах, если
   ...
   полный GC между файлами через трюк `v8.setFlagsFromString('--expose-gc')`
   без необходимости запускать Node с этим флагом. Регресс-тест:
   `tests/multi-file-scan.test.ts`.
